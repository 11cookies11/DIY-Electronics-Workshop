...
## 10. 2026-03 自然对话式改造补充

当前前台 Agent 已经从“用户显式触发 preview / handoff”进一步改成“先正常聊天，系统内部持续准备候选结果”的模式：

- 对用户侧：
  - 先正常对话，不要求用户先说出固定设备类别
  - Agent 先接住需求，再自然邀请进入 preview 或 handoff
- 对系统侧：
  - 对话过程中持续提炼 `preview_candidate` 和 `handoff_candidate`
  - 候选结果已生成，不代表已经展示给用户
  - 真正展示时，仍然由 readiness 与用户推进信号共同决定

这意味着当前状态流里要区分三件事：

- `candidate ready`
  - 内部已经能拼出 preview 或 handoff
- `offering`
  - 前台开始自然邀请用户看预览或看交接稿
- `exposed`
  - `preview_input_draft` 或 `lab_handoff` 已经正式对前端暴露

## 11. 内部原型推断

当前追问顺序不再只依赖用户是否明确说出“遥控器 / 手持设备 / 桌面设备”。

系统会根据对话里提到的使用方式、控制对象、摆放位置、交互方式、音频特征、穿戴特征等信息，在内部推断设备原型，例如：
...
- 决定下一轮优先追问什么
- 在用户没说类别名时，辅助推断 `device_type`
- 让 preview readiness 更早从自然语言中成立

现在像“放客厅茶几上、拿在手里控制电视和投影、保留实体按键”这类说法，即使用户没说“遥控器”，系统也能自动往遥控器式 intake 路线推进。
